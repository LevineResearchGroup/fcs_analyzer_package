...
def average_data(path, files):
    '''Imports data and calculates average using get_error above (error propagation)'''
    sample_size = 3   #FIXME
    new_list = []
...
import os
path = './Results/'
D1c_files = './Results/*D1c_AA.dat'
Rh1c_files = './Results/*Rh1c_AA.dat'

D1c = []
Rh1c = []

#Import files with diffusion constants and Rh values for 1 component model.
for name in glob.glob(path + "*D1c_AA.dat"):
    name = os.path.basename(name)
    D1c = np.append(D1c, name)
    
for name in glob.glob(path + "*Rh1c_AA.dat"):
    name = os.path.basename(name)
...
# Rh4 = Import_data(path, Rh1c[6], Rh1c[7])
# Rh4 = Import_data(path, Rh1c[8], Rh1c[9])
