...
####FIXME####
def simplefit(t, G, err, kappa = 5, triplet = 1, start = 0):
    ...
    if triplet == 1:
        ...
        params['T'].set(min=0, max=1, value=0, vary=False)
    params['A0'].set(min=0.00001, value=A0_guess)
    params['tau_diff'].set(min=1e-6, value=tau_guess)
    params['tau_trip'].set(min=1e-6, max=1e-5, value=20e-6)
    params['Ginf'].set(value=0, vary = True)
    params['kappa'].set(value=kappa, vary=False)  # 3D model only
...
path = "/Users/bab226/Documents/yale_research/iapp/fcs/fcs-analysis-package/Data/BB_dextran_mixtures_10_20_21.sptw/"
for name in glob.glob(path + "fluorescin_na_10-15-21.dat"):
    ...
    # for m in measurement_group[::2]: #everyother meauserment    
        ...
        fitres1c = simplefit(t, G, err, kappa = set_kappa, triplet = 1, start = 0)
        ...
        tau_trip_1c.apped(fitres1c.values['tau_trip'])
        tau_trip_1c_err.apped(fitres1c.values['tau_trip'].stderr)
        T_1c.append(fitres1c.values['T'])
        T_1c_err.append(fitres1c.values['T'].stderr)
