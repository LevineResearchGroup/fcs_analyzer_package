...
def plotBars(x,y,ylim,bar_labels,filename):
    ...
    for i in range(len(x)):
        ...
        ax.scatter(x[i] + np.random.random(len(y)) * (w*spread) - (w / 2) + w*space, y[i], color="black", s=100, marker='*', zorder=5)
...
name = ['iao_sds_A_21C_01-11-22', 'iao_sds_B_21C_01-11-22', 'iao_sds_C_21C_01-11-22']
...
ylim = np.array([350,3,10])    #[D, Rh, N ...]

bar_labels = np.array(['0.2%', '0.1%', '0.025%'])
save_text = 'sds_iao_oligomers'       #text used for file naming
...
for i in range(0,len(strings)):
    ...
    for s in range(0,len(name)):
        ...
        t1, s1, err_s1 = rd.ReadData(path + name[s] + strings[i] + key + '.dat')
        y.append([s1])
