...
path = "/Users/bab226/Documents/yale_research/FRET-FCS/FRET-FCS-toshare-05-13-2021/Data/BB_IAPP_SEC_fractions_08-17-21.sptw/"
for name in glob.glob(path + "*.dat"):
    ...
    td_ref = ufloat(0.0336, 0.0336) #from calibration (ms)
    D_ref = ufloat(470, 40) #from literature, for calibration (um^2/s), R110
    temperature_ref = ufloat(25, 0.5) # temperature at which reference D was taken (celsius)
    ...
    """ %(name, np.mean(tau_fast2c), np.std(tau_fast2c, ddof=1), np.mean(tau_slow2c), np.std(tau_slow2c, ddof =1), np.mean(D1c), np.std(D1c, ddof =1), np.mean(D2c), np.std(D2c, ddof =1), np.mean(Rh2c), np.std(Rh2c, ddof =1), np.mean(p1), np.std(p1, ddof=1), np.mean(redchi2c), np.std(redchi2c, ddof =1)), file=f)
    
    
    #Diffusion Time
    with open('./Results/' + name + '_Dt_' + key + '.dat', "w" ) as f:
        f.write('t \t T \t err \n')
        for t,R,err in zip(t_measurement, tau_1c, tau_err1c):
            f.write('%.3f \t %.3f \t %.3f \n' %(t,R,err))
    ...
    """ %(name, np.mean(tau_1c), np.std(tau_1c, ddof =1), np.mean(D1c), np.std(D1c, ddof =1), np.mean(Rh1c), np.std(Rh1c, ddof =1), np.mean(redchi1c), np.std(redchi1c, ddof =1)), file=f)
